...
This could be because some other process is modifying AWS at the same time."""
    ...
    class Aws < Puppet::Provider
      ...
      def self.read_only(*methods)
        methods.each do |method|
          define_method("#{method}=") do |v|
            Puppet.debug "#{method} property is read-only once #{resource.type} created."
          end
        end
      end
      ...
      def self.acm_client(region = default_region)
        ::Aws::ACM::Client.new(client_config(region))
      end

      def acm_client(region = default_region)
        self.class.acm_client(region)
      end
